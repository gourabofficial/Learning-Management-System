...
const Player = () => {
  ...
  const { enrolledCourses, calculateChapterTime
    , backendUrl, getToken, fetchUserEnrolledCourses } = useContext(AppContext)
  const { courseId } = useParams()
  const [courseData, setCourseData] = useState(null)
  const [openSectios, setOpenSections] = useState({})
  ...
  return courseData ? (
    <>
      <div className="p-4 sm:p-10 flex fflex-col-reverse md:grid md:grid-cols-2
      gap-10 md:px-36">
        {/* left side  */}
              ...
              <div>
                ...
                <div className = "flex justify-between items-center mt-1 " >
                  ...
                  <button onClick={() => markLectureAsCompleted(playerData.lectureId)} className='text-blue-600'>{progressData && progressData.
                    lectureCompleted.includes(playerData.lectureId) ? 'completed' : "Mark Completed"}</button>
                </div>
              </div>
